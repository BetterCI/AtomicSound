function [cf,r,gt] = gammatoneFast(in, numChan, fRange, fs)
...
erb = [erb_b(1):diff(erb_b)/(numChan):erb_b(2)];     % ERB segment
for n = 1:numChan
    cf(n) = mean(erb2hz(erb(n:n+1)));
end
